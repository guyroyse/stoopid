describe('Object', function() {
  ...
  describe('forEach', function() {
    ...
    it('passes the values to the callback function', function() {
      ...
      expect(values).toEqual([42, true, 'bob']);
    });
  ...
  describe('filter', function() {
    ...
    it('passes the values to the callback function', function() {
      var values = [];
      subject.filter(function(key, value) {
        values.push(value);
      });
      expect(values).toEqual([42, true, 'bob']);
    });
    ...
    it('passes the object to the callback function', function() {
      var callbackObject = {};
      subject.filter(function(key, value, object) {
        callbackObject = object;
      });
      expect(callbackObject).toBe(subject);
    });

    it('returns empty object when callback returns false', function() {
      var response = subject.filter(function() {
        return false;
      });
      expect(response).toEqual({});
    });

    it('returns full object when callback returns true', function() {
      var response = subject.filter(function() {
        return true;
      });
      expect(response).toEqual( { bar: 42, baz: true, qux: 'bob' } );
    });

    it('returns items that match filter', function() {
      var response = subject.filter(function(key) {
        return key === 'bar' || key === 'qux';
      });
      expect(response).toEqual( { bar: 42, qux: 'bob' } );
    });
